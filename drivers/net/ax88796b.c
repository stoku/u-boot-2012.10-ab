...
{
	return (*((volatile unsigned char *)(dev->iobase + regno)));
}
...
static int ax88796b_recv (struct eth_device *dev)
{
	...
	if (!(status & 1)) {
		if (!ax_local->wait_link) {
			...
			/* power up phy */
			mdio_write (dev, 0x10, MII_BMCR, BMCR_ANENABLE);
...
static int ax88796b_send (struct eth_device *dev, void *packet, int length)
{
	unsigned int i;
